...
let rec gen = function
  ...
  | Exp_fun (args, e) ->
      let* as_pat, bounds_pat, tys_pat = Pattern.gen_many (from_list1 args) in
      let* as_e, ty_e =
  ...
  | Exp_tuple exprs ->
      let* asm, tys = gen_many (from_list2 exprs) in
      return (asm, Ty.Ty_tuple (to_list2_exn tys))
  ...
  | Exp_match (e, cases) ->
      ...
      let* ty_res = fresh_var >>| ( ! ) in
      let* as_cases =
        GenMonad.List.fold (from_list1 cases) ~init:As.empty ~f:(fun acc case ->
