...
let parse_exp_let pexp =
  lift2
    ...
    (parse_let_binding pexp parse_pattern parse_ty)
    (ws *> string "in" *> pexp)
...
let parse_exp_list pexp =
  let parse_list =
    ...
    let rec helper = function
      | h :: tl ->
          Exp_construct
            (Ident "::", Some (Exp_tuple (to_list2_exn [h; helper tl])))
      | [] ->
...
(** [E0 : T] *)
let parse_exp_constraint pexp =
  lift2
    (fun exp typ -> Exp_constraint (exp, typ))
    (char '(' *> pexp)
    (ws *> char ':' *> Ty.parse_ty <* ws <* char ')')
...
let parse_exp_match pexp_match pexp_with =
  lift2
    ...
    (ws *> string "with" *> ws *> parse_match_cases pexp_with >>| to_list1_exn)
  ...
  let rec pexp disabled_op =
    let fold_infix acc (op, rhs) =
      ...
      | IOpTuple -> (
        match rhs with
        | Exp_tuple l ->
            Exp_tuple (to_list2_exn (acc :: from_list2 l))
        | _ ->
